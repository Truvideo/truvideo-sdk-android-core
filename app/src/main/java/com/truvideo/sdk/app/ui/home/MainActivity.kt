...
//        filePicker = TruvideoSdk.initFilePicker(this)
    ...
    fun Content() {
        ...
        val isAuthenticated by remember {
            mutableStateOf(sdk_common.auth.isAuthenticated)
        }
        val isAuthenticatedInitialized by remember {
            mutableStateOf(sdk_common.auth.isInitialized)
        }
        val isAuthenticationExpired by remember {
            mutableStateOf( sdk_common.auth.isAuthenticationExpired)
        }
        val currentApiKey by remember {
            mutableStateOf(sdk_common.auth.apiKey)
        }
        val authentication by remember {
            mutableStateOf(sdk_common.auth.authentication)
        }
        val settings by remember {
            mutableStateOf(sdk_common.auth.settings)
        }
