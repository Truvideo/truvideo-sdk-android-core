...
class MainActivity : ComponentActivity() {
    ...
    private lateinit var filePickerRequester: TruvideoSdkFilePickerRequester
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        filePickerRequester = TruvideoSdk.initFilePickerRequester(this)
    ...
    fun Content() {
        ...
        fun requestFilePicker(type: TruvideoSdkFilePickerType) {
            scope.launch {
                try {
                    val path = filePickerRequester.pick(type)
                    pickedFile = path ?: ""
                } catch (exception: Exception) {
                    exception.printStackTrace()
                }
