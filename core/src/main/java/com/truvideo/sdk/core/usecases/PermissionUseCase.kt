...
internal class PermissionUseCase {
    ...
    private var requesters = mutableMapOf<ComponentActivity, TruvideoSdkPermissionRequester>()

    fun init(activity: ComponentActivity): TruvideoSdkPermissionRequester {
        var requester = requesters[activity]
        if (requester == null) {
            requester = TruvideoSdkPermissionRequester(activity)
...
class TruvideoSdkPermissionRequester(private val activity: ComponentActivity) {
